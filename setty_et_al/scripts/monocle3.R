...
trash <- capture.output(
  ...
  for (i in azi_obj_name) {
    ...
    cds <- new_cell_data_set(
      expression_data = s.obj@assays$RNA@counts,
    ...
    p <- plot_cells(cds,
      reduction_method = "UMAP", cell_size = 2,
      color_cells_by = "az_labels", label_cell_groups = T
    ) +
      scale_color_brewer(palette = "Dark2") +
      ggtitle(paste("Replicate:", names(rep_split), "|", "Age:", age, "|", " Gender:", gen)) +
      theme(
        legend.position = "bottom", axis.text = element_text(size = rel(3)), legend.title = element_blank(),
    ...
    p1 <- plot_cells(cds,
      ...
      ggtitle(paste("Replicate:", names(rep_split), "|", "Age:", age, "|", " Gender:", gen)) +
      theme(
        legend.position = "bottom", axis.text = element_text(size = rel(3)),
    ...
    p2 <- plot_cells(cds,
      reduction_method = "UMAP", cell_size = 2, trajectory_graph_segment_size = 3,
    ...
    for (j in c(mds_genes, oth_genes)) {
      ...
      p <- plot_genes_in_pseudotime(cds_sub,
        color_cells_by = "az_labels", cell_size = 3,
