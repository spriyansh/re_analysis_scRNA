...
# Save
write.table(as.data.frame(auto.res), sep = "\t",
...
for (i in trans_genes){
    ...
    ggsave(p, filename = paste0(prefixOut, "/Transit_UMAP/", i,".png"),
...
for (i in mds_genes){
    ...
    p <- plot_cells(cds, reduction_method = "UMAP", cell_size = 0.5,
        ...
        theme(plot.margin = margin(0.2,0.2,0,0, "cm"),
              ...
              axis.title = element_text(size = rel(1)), legend.text = element_text(size = rel(1)),
              legend.key.size = unit(0.5, "cm"), plot.title = element_text(size = rel(1)))
    ggsave(p, filename = paste0(prefixOut, "/MDS_UMAP/", i,".png"),
           dpi = 1300, limitsize = FALSE
    )
}
...
for (i in LM_genes){
    ...
    p <- plot_cells(cds, reduction_method = "UMAP", cell_size = 0.5,
        ...
        theme(plot.margin = margin(0.2,0.2,0,0, "cm"),
              ...
              axis.title = element_text(size = rel(1)), legend.text = element_text(size = rel(1)),
              legend.key.size = unit(0.5, "cm"), plot.title = element_text(size = rel(1)))
    ggsave(p, filename = paste0(prefixOut, "/LM_UMAP/", i,".png"),
           dpi = 1300, limitsize = FALSE
    )
}
...
for (i in trans_genes){
    ...
    ggsave(p, filename = paste0(prefixOut, "/Transit_Trend/", i,".png"),
           dpi = 1300, limitsize = FALSE
    )
}
...
for (i in mds_genes){
    ...
    p <- plot_genes_in_pseudotime(cds[rowData(cds)$gene_short_name == i,],
        ...
        theme(plot.margin = margin(0.2,0.2,0,0, "cm"),
              ...
              legend.key.size = unit(0.5, "cm"), plot.title = element_text(size = rel(1))) +
        guides(colour = guide_legend(override.aes = list(size = rel(6))))
    ggsave(p, filename = paste0(prefixOut, "/MDS_Trend/", i,".png"),
           dpi = 1300, limitsize = FALSE
    )
}
...
for (i in LM_genes){
    ...
    p <- plot_genes_in_pseudotime(cds[rowData(cds)$gene_short_name == i,],
        ...
        theme(plot.margin = margin(0.2,0.2,0,0, "cm"),
              ...
              legend.key.size = unit(0.5, "cm"), plot.title = element_text(size = rel(1))) +
        guides(colour = guide_legend(override.aes = list(size = rel(6))))
    ggsave(p, filename = paste0(prefixOut, "/LM_Trend/", i,".png"),
           dpi = 1300, limitsize = FALSE
    )
}
