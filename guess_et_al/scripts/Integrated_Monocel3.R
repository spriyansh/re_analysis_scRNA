...
cds_temp <- reduce_dimension(cds_temp,
  preprocess_method = "PCA", max_components = 2,
...
ggsave(p,
  filename = paste0(prefixOut, "/Unintegrated/Year.png"),
...
# Comparison
umap.df <- data.frame(
  cell.type = as.factor(colData(cds)$cell.type),
...
p1 <- ggplot(umap.df, aes(
  x = umap1, y = umap2, alpha = orig.ident,
  color = cell.type
)) +
  geom_point(data = umap.df, size = 0.5, shape = 16) +
  theme_classic() +
  ggtitle("Patient-17: MDS State") +
...
p2 <- ggplot(umap.df, aes(
  x = umap1, y = umap2, alpha = orig.ident,
  color = cell.type
)) +
  geom_point(data = umap.df, size = 0.5, shape = 16) +
  theme_classic() +
  ggtitle("Patient-17: sAML State (After Two Years)") +
...
dir.create(paste0(prefixOut, "/Integrated/"), showWarnings = FALSE)
ggsave(p,
  filename = paste0(prefixOut, "/Integrated/StateComparison.png"),
